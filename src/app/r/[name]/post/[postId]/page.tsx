...
const PostPage = async ({ params: { postId } }: IPostPageProps) => {
  const { data, status } = await serverAxios().get<TApiRes<PostResponse>>(`/api/post/${postId}`);
  ...
  const getPostData = async (): Promise<DBPost | null> => {
    ...
    const { data } = await serverAxios().get<TApiRes<PostResponse>>(`/api/post/${postId}?force=true`);
