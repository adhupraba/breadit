...
const CommentsSection = async ({ postId }: ICommentsSectionProps) => {
  ...
  const { data, status } = await serverAxios().get<TApiRes<CommentWithReplies[]>>(`/api/comment/post/${postId}`);
    ...
    <div className="flex flex-col gap-y-4 mt-4">
      ...
      <div className="flex flex-col gap-y-6 mt-4">
        {comments.map((comment) => {
          ...
          console.log("comment", comment.text, { cmntVote, cmntVoteCount });
          ...
          return (
            <div key={comment.id} className="flex flex-col">
              ...
              {comment.replies.map((reply) => {
                ...
                console.log("reply", reply.text, { replyVote, replyVoteCount });
